...
- `ports[]` and `compatibility[]` are the **port-typed registry** layer (the abstract port contracts + the derived feature-model edges). They are additive — a `contract_version: 1` consumer that doesn't resolve them simply ignores them; a resolver consumes them to validate a verified configuration. See [`MANIFEST_SCHEMA.md`](MANIFEST_SCHEMA.md) `### ports[]` / `### compatibility[]`.
- `recipes[].bindings`, `recipes[].context_manifest`, and `capabilities[].context_summary` are **derived consumer aids** (generator-emitted, additive): the resolved port → adapter map, the closed pre-costed context set (load exactly it and skip speculative discovery), and a compact per-adapter summary to inject instead of the full body. Ignorable by a `contract_version: 1` consumer that doesn't use them.
