...
Pydantic AI ships first-class MCP client support via `pydantic_ai.mcp.MCPToolset`, built on the FastMCP client. Pass any input FastMCP can build a transport from (a URL for streamable HTTP, a script path or command for stdio); tools discovered from connected servers join the agent's tool surface alongside `@agent.tool` Python-decorated tools. The 1.x-era `MCPServerStreamableHTTP` / `MCPServerStdio` classes, the `mcp_servers=` keyword, and the `agent.run_mcp_servers()` context manager are gone in current releases.
...
`agent.run()` manages the toolset lifecycle itself: transports open, tools are discovered and registered, and everything closes when the run completes.
...
postgres = MCPToolset(
    {"command": "npx", "args": ["-y", "@modelcontextprotocol/server-postgres", os.environ["DATABASE_URL"]]}
)

agent = Agent("anthropic:claude-sonnet-4-6", toolsets=[tavily, postgres])
```
