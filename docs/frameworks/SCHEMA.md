...
| `## MCP integration` | `mcp-integration` | Required | End-to-end wiring example: client init, server discovery, tool binding, invocation. Both `stdio` and `streamable_http` transports where the framework supports them. Mirrors the `mcp_servers[]` shape declared in [`../recipes/SCHEMA.md`](../recipes/SCHEMA.md). Consumers loading a recipe with `mcp_servers[]` read this section to learn how to wire the listed MCP capabilities into the framework's idiomatic agent surface. |
