...
A task runner for agent projects. Supports dual-track (Python/TypeScript) with Docker Compose, testing tiers, eval, linting, and security scanning.

## Key design decisions

- **`PROTOTYPE` and `TRACK` variables** — all targets are parameterized so a single Makefile serves the entire repo
- **Three-tier testing** — `test-unit`, `test-integration`, `eval` map to the [testing strategy](../cross-cutting/testing-strategy.md)
- **Exit code 5 handling** — `|| test $$? -eq 5` treats "no tests collected" as success for prototypes without integration/eval tests yet
- **Security scanning** — runs Promptfoo red-team against both tracks
...
- Replace `prototypes/$(PROTOTYPE)/$(TRACK)` paths with your project structure
- If single-language, remove the `ifeq ($(TRACK),python)` conditionals
- Add `deploy` targets for your specific cloud provider
