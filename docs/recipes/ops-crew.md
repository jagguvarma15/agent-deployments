...
acceptance_contracts:
  http_endpoints:
    ...
    - {path: /investigate, method: POST, status: 200}
  required_env:
  ...
  smoke_assertions:
    - {jq: '.report | length > 0', against: smoke_test.exercise.stdout}
...
- Pattern: [Multi-Agent Flat](https://github.com/jagguvarma15/agent-blueprints/blob/main/patterns/multi_agent/overview.md)
...
- `https://github.com/jagguvarma15/agent-blueprints/blob/main/patterns/multi_agent/overview.md` — the multi-agent flat pattern
...
- `docs/stack/relational-postgres.md` — ops report persistence
...
Step 1 — Fetch:
  ...
  - https://raw.githubusercontent.com/jagguvarma15/agent-deployments/main/docs/recipes/ops-crew.md
  - Every `load_list[].path` with `required: true` and `cache_tier: hot`.

Step 2 — Generate the project at `./ops-crew/` matching the recipe's `required_files[]`:
  - model(s): devops=claude-sonnet-4-6, security=claude-sonnet-4-6, database=claude-sonnet-4-6
  - framework: crewai (Python) or mastra (TS)
  - runtime_mode: default
  - env vars: from `catalog.recipes[ops-crew].env_contract`
...
Step 5 — Validate against `catalog.recipes[ops-crew].acceptance_contracts`.
````
...
|-----------|-----------|-----|
| Postgres | Yes | Ops report and finding persistence |
| Redis | Yes | Rate limiting backend |
| Qdrant | No | Not needed — agents query live infrastructure, not documents |
| Langfuse | Recommended | Multi-agent call tracing (skip for local dev) |
